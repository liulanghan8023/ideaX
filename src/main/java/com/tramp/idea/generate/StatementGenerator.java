...
import com.tramp.idea.service.EditorService;
import com.tramp.idea.service.JavaService;
import com.tramp.idea.setting.MybatisSetting;
import com.tramp.idea.ui.ListSelectionListener;
import com.tramp.idea.ui.UiComponentFacade;
import com.tramp.idea.util.CollectionUtils;
...
public abstract class StatementGenerator {
    ...
    public static final StatementGenerator INSERT_GENERATOR = new InsertGenerator("insert", "add", "new", "save");
    ...
    public static void applyGenerate(@Nullable final PsiMethod method) {
        ...
        if (1 == generators.length) {
            ...
            JBPopupFactory.getInstance().createListPopup(new BaseListPopupStep("[ Statement type for method: " + method.getName() + "]", generators) {
                @Override
    ...
    private void setupTag(PsiMethod method, Mapper mapper) {
        ...
        if (psiParameters.length > 0) {
            PsiParameter psiParameter = psiParameters[0];
            PsiClass psiClass = PsiTypesUtil.getPsiClass(psiParameter.getType());
            target.getParameterType().setValue(psiClass);
        }
